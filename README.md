...
在 Windows 助手中选择文件并点击生成日报时，程序会先检查文件的最后修改日期。如果文件不是今天更新的，会在发送前弹出警告并默认停下；只有明确确认后才会继续使用旧文件，避免误发上一天的数据。命令行一键日报也会在更新、制图和发送前停止；确实需要处理旧文件时，必须显式添加 `--allow-old-source`。
...
如果多个日报图片配置实际发送到同一个群，不要删除 `messages` 中的图片任务。可以只在文字多选
列表中把它们合并：
...
程序会确认同组成员的发送方式和接收目标完全一致，再把它们显示为一个选项并且只发送一次；
原来的日报图片任务保持不变。修改配置后请重启 Windows 助手，使界面和发送流程使用同一份列表。
同一个 `Send as me` 接收群如果对应多条图片任务，也可以把这些名称全部放入同一组，避免手动
多选时向同一个群重复发送文字。
